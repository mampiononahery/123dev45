                ...
                success: function (data) {
                    ...
                    $.each(data, function (key, value) {
                        if (value.produit) {
							...
							var length_produit = produits.length;
                            var total = 0;
                            var selected = 0;
							...
							var facturation = '<a href="<?php echo site_url("user/ajax/imprimer_pdf")?>/'+event_id+'" class="btn btn-large" style="margin-top: 5px;margin-bottom: 5px;margin-left: 10px;" title="Facturation" role="button" aria-label="Facturation"> <i  class="fa fa-print"></i> Imprimer</a>';
							if(length_produit>0){
								_table+=facturation;
						   }
							$("#selectProduct").html(_table);
...
<?php endif; ?>
		...
		scheduler.attachEvent("onDragEnd", function(event,mod){
			...
			*/
			...
			if(mod=="move"){
				var x = new Date();
                dragged_event.start_date = new Date(dragged_event.start_date.getTime()-x.getTimezoneOffset()*60*1000);
				dragged_event.end_date = new Date(dragged_event.end_date.getTime()-x.getTimezoneOffset()*60*1000);
				console.log(dragged_event);
				$.ajax({
					url: "<?php echo site_url('user/ajax/save_event') ?>",
					type: "POST",
        ...
        scheduler.renderEvent = function (container, ev, width, height, header_content, body_content) {
            ...
            if (ev.sms === "1") { 
				...
				if(ev.log_id==null)
				{
					 html += '<img class="sms_scheduled" src="<?php echo site_url('assets/backend/design/1.png') ?>" title="SMS planifié" alt="SMS planifié"/>';
				}
				else
				{
					 html += '<img class="sms_sent" src="<?php echo site_url('assets/backend/design/2.png') ?>" title="SMS de notification de RDV envoyé" alt="SMS de notification de RDV envoyé"/>';
				}
