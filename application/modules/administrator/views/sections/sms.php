		...
		if($this->input->post("periode"))
		{
		    ...
		    switch($this->input->post("periode")){
				...
				case 1 :
					...
					$crud->where("MONTH(sending_date)='".$mois."' AND YEAR(sending_date) = ".$year);
						
				break;
				
				case 2:
					$crud->where("MONTH(sending_date)<='".$mois."' AND MONTH(sending_date)>='".$mois_6."'  AND YEAR(sending_date) = ".$year);
				break;
					
				
				case 3:
					$crud->where("YEAR(sending_date) = ".$year);
				break;
			...
			if($date_debut!=""){
				...
				$crud->where("sending_date >='".$date_debut."'");
			}
			
			if($date_fin!=""){
				
				$crud->where("sending_date <='".$date_fin."'");
			}
		}
        $crud->display_as(array(
            'rdv_id' => 'Rendez-vous',
