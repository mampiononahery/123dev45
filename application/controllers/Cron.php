...
class Cron extends Front {
	...
	public function generete_pdf(){
		
		$html = "<p>Bonjour</p>";
		generate_pdf($html,"test","A4","A4-L");
...
//echo $campaign->user;			
            ...
            $query = json_decode($current_request->data_posted,true);
//print_r($query);
            if (!empty($query)) {
...
//print_r($table_value);
                ...
                $result = $this->_launch_cron_query($table_selection, $table_field, $table_condition, $table_value,$campaign->user);
//                print_r($result);
...
//print_r($data);
    ...
    function _launch_cron_query($table_selection, $table_field, $table_condition, $table_value, $cron_uid) {
        $this->load->model('Client_model');
        $this->load->model('Rdv_model');
