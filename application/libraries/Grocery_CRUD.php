...
/*           ob_flush();
  ...
  */         
           if(!empty($post_data["heure"]))
           {
             $heure = $post_data["heure"];
           }
           ...
           if ($this->config->xss_clean) {
            ...
            if ($this->callback_insert === null) {
                ...
                if($heure!="")
                {
                $insert_data["sending_date"] = $insert_data["sending_date"]." ".$heure.":00";
                //                                              file_put_contents("/tmp/log_powerCRM", $update_data["sending_date"]);
...
*/			
            ...
            if ($this->callback_update === null) {
				...
				if(!empty($heure))
				{
					if($heure!="")
					{
						$update_data["sending_date"] = $update_data["sending_date"]." ".$heure.":00";
//						file_put_contents("/tmp/log_powerCRM", $update_data["sending_date"]);
                date_default_timezone_set('UTC');
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver {
	...
	public function get_longuer_messega($message)
	{
    ...
    protected function showList($ajax = false, $state_info = null) {
		...
		if($table_name=="marketing_sms_logs"){
			$data_retour = array();
			$data_temp = $this->get_list();
			foreach($data_temp as $t){
			
				$t->total_sent = calcul_nombre_message($t->message);
				$data_retour[] = $t;
			
			}
			$data->list  = $data_retour;
		...
		//var_dump($this->basic_model->get_table_name());
    ...
    protected function _export_to_excel($data) {
        ...
        $string_to_export = "";
		  ...
		  $ci = &get_instance();
		$lis_col = $ci->session->userdata("columns");
		...
		{
				if(!in_array($col->field_name,$lis_col))
				{
					$temp[] = $col;
				}
    ...
    protected function get_datetime_input($field_info, $value) {
		...
		$temp = $datetime ;
		if($field_info->name=="sending_date"){
			...
			$input .= "<select id='field-heure' name='heure' class='chosen-select' style='width:120px;' data-placeholder='HH:mm'>";
			$options = array('' => '',
				"08:00" => "08:00",
				"08:15" => "08:15",
				"08:30" => "08:30",
			...
			foreach ($options as $option_value => $option_label) {
				$selected = !empty($valueH) && $valueH == $option_value ? "selected='selected'" : '';
			...
			//$input .= "<a class='datepicker-input-clear' tabindex='-1' style='margin-left:5px'>" . $this->l('form_button_clear') . "</a>";
			return $input;
		}
		else{
